...
#' @param LOO.test.size How many forecasts from the past to
#' generate leave-one-out AnEn test results for. These historical forecasts 
#' are sorted based on the similarity to the current forecasts.
...
heuristicFilter <- function (AnEn, config, final.ensemble.size,
  ...
  summary <- tabulate(AnEn$similarity[i.station, i.test.day, , 1:config$num_members, 3])
